...
void PipelineGraphics::CreateAttributes() {
	...
	//auto _renderManager = Graphics::Get()->GetRenderStage(stage.first);
	bool multisampled = false; // _renderManager->IsMultisampled(stage.second);
...
void PipelineGraphics::CreatePipeline() {
	...
	pipelineCreateInfo.renderPass = *_renderManager->GetRenderpass();
...
void PipelineGraphics::CreatePipelineMrt() {
	auto _renderManager = Graphics::Get()->GetRenderStage(stage.first);
	auto attachmentCount = _renderManager->GetAttachmentCount(stage.second);
