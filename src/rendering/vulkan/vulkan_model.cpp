...
namespace std {
template <>
struct hash<Avarice::VulkanModel::Vertex> {
  size_t operator()(Avarice::VulkanModel::Vertex const &vertex) const {
    size_t seed = 0;
    Avarice::hashCombine(seed, vertex.position, vertex.color, vertex.normal, vertex.uv);
    return seed;
  }
};
}  // namespace std
...
VulkanModel::VulkanModel(VulkanDevice &device, const VulkanModel::Builder &builder) : m_device{device} {
