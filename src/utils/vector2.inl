...
namespace std {
template<typename T>
struct hash<Avarice::Vector2<T>> {
	size_t operator()(const Avarice::Vector2<T> &vector) const noexcept {
		size_t seed = 0;
		Avarice::Maths::HashCombine(seed, vector.x);
		Avarice::Maths::HashCombine(seed, vector.y);
		return seed;
	}
};
}
