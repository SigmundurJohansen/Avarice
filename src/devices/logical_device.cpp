...
namespace Avarice
{
	...
	LogicalDevice::LogicalDevice(const Instance *instance, const PhysicalDevice *physicalDevice, const Surface *surface) : instance(instance),
	...
	void LogicalDevice::CreateLogicalDevice()
	{
		...
		{
			printf("Selected GPU does not support wireframe pipelines!\n");
		}

		if (physicalDeviceFeatures.samplerAnisotropy)
			enabledFeatures.samplerAnisotropy = VK_TRUE;
		else
			printf("Selected GPU does not support sampler anisotropy!\n");
		...
		if (physicalDeviceFeatures.vertexPipelineStoresAndAtomics)
			...
			printf("Selected GPU does not support vertex pipeline stores and atomics!\n");

		if (physicalDeviceFeatures.fragmentStoresAndAtomics)
			enabledFeatures.fragmentStoresAndAtomics = VK_TRUE;
		else
			printf("Selected GPU does not support fragment stores and atomics!\n");

		if (physicalDeviceFeatures.shaderStorageImageExtendedFormats)
			enabledFeatures.shaderStorageImageExtendedFormats = VK_TRUE;
		else
			printf("Selected GPU does not support shader storage extended formats!\n");

		if (physicalDeviceFeatures.shaderStorageImageWriteWithoutFormat)
			enabledFeatures.shaderStorageImageWriteWithoutFormat = VK_TRUE;
		else
			printf("Selected GPU does not support shader storage write without format!\n");
		...
		if (physicalDeviceFeatures.geometryShader)
			...
			printf("Selected GPU does not support geometry shaders!\n");

		if (physicalDeviceFeatures.tessellationShader)
			enabledFeatures.tessellationShader = VK_TRUE;
		else
			printf("Selected GPU does not support tessellation shaders!\n");

		if (physicalDeviceFeatures.multiViewport)
			enabledFeatures.multiViewport = VK_TRUE;
		else
			printf("Selected GPU does not support multi viewports!\n");
